...
# ==============================================================================
"""
Modified by Jhonatan Contreras, Aug 2021.
...
from datasets import input_preprocess
...
class Dataset(object):
    ...
    def _preprocess_image(self, image, label):
        ...
        Args:
           image, label: A sample containing image and label.

        Returns:
          image, label: Sample with preprocessed image and label.
    ...
    def get_dataset(self):
        ...
        dataset = tf.data.TFRecordDataset(files).map(self._parse_function).map(
                                                 self._preprocess_image)
        if self.should_shuffle:
            dataset = dataset.shuffle(buffer_size=100)

        if self.should_repeat:
            dataset = dataset.repeat(2)  # Repeat forever for training.
