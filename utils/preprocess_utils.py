...
def flip_dim(tensor_list, prob=0.5, dim=1):
  ...
  is_flipped =  tf.math.less_equal(random_value, prob)
...
def randomly_scale_image_and_label(image, label=None, scale=1.0):
  ...
  image = tf.squeeze( tf.image.resize(tf.expand_dims(image, 0), new_dim), [0])

  if label is not None:
    label = tf.image.resize(
        label,
        new_dim,
        method=get_label_resize_method(label))#,   align_corners=True)
